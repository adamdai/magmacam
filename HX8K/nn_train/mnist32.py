...
def extract_images(f):
    ...
    with gzip.GzipFile(fileobj=f) as bytestream:
        ...
        resized_data = numpy.zeros([num_images, 32, 32, 1])
        for i in range(num_images):
            resized_img = cv2.resize(data[i, ..., 0], (32, 32))
            resized_data[i, ..., 0] = resized_img
...
class DataSet(object):
    ...
    def __init__(self,
        ...
        `uint8` to leave the input as `[0, 1023]`, or `float32` to rescale into
